...
export default function ProductCard({ product }: Props) {
  ...
  const togglePanel = () => {
    if (!open) {
      ...
      const svgOpen = btnRef.current?.querySelector('svg')
      if (svgOpen) gsap.to(svgOpen, { rotate: 180, duration: 0.35, ease: 'power2.out' })
      ...
      const svgClose = btnRef.current?.querySelector('svg')
      if (svgClose) gsap.to(svgClose, { rotate: 0, duration: 0.3, ease: 'power2.in' })
    }
  }
