...
export default function AdminShell({ children }: { children: React.ReactNode }) {
  ...
  const Sidebar = () => (
    <aside className="flex flex-col h-full w-60 bg-[#0d1117] border-r border-white/10">
      ...
      <div className="px-3 py-4 border-t border-white/10">
        {email && (
          <div className="px-3 py-2 mb-2 bg-white/5 rounded-md">
            <p className="text-[10px] text-gray-500 mb-0.5">Sesión activa</p>
  ...
  return (
    <div className="flex min-h-screen bg-[#111624]">
      {/* Desktop sidebar */}
      <div className="hidden md:flex md:flex-col md:fixed md:inset-y-0 md:left-0 md:w-60 z-30">
        <Sidebar />
      </div>
      ...
      {/* Mobile drawer */}
      <div className={`fixed inset-y-0 left-0 z-50 w-60 flex flex-col md:hidden
      ...
      <div className="flex-1 md:ml-60 flex flex-col min-h-screen">
        ...
        <div className="md:hidden flex items-center justify-between
                        px-5 py-4 border-b border-white/10 bg-[#0d1117]">
          <button onClick={() => setOpen(true)} className="text-gray-400 hover:text-white">
            <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M3.75 6.75h16.5M3.75 12h16.5m-16.5 5.25h16.5" />
            </svg>
          </button>
          <span className="text-sm font-medium text-white">Studio G.D. Admin</span>
          <div className="w-5" />
        </div>
